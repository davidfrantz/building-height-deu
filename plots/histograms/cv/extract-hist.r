...
dirs1 <- c(
"/data/Jakku/germany-height/3d-extract/maps/berlin",
"/data/Jakku/germany-height/3d-extract/maps/hh",
"/data/Jakku/germany-height/3d-extract/maps/nrw",
"/data/Jakku/germany-height/3d-extract/maps/potsdam",
"/data/Jakku/germany-height/3d-extract/maps/thueringen",
"/data/Jakku/germany-height/3d-extract/maps/vienna")
...
for (site in 1:6){
  ...
  for (t in 1:nt){
    ...
    if (!file.exists(sprintf("%s/%s/3D-HEIGHT.tif", dirs1[site], tiles[t]))){
      ...
      z[[t]] <- matrix(raster(sprintf("%s/%s/3D-HEIGHT.tif", dirs1[site], tiles[t]))[], 3000, 3000)
    }

    z_[[t]] <- matrix(raster(sprintf("%s/%s/BUILDING-HEIGHT_CV_HL_ML_MLP.tif", dirs2[site], tiles[t]))[]/10, 3000, 3000)
...
hist(z_[z>0],  breaks=seq(-100,1000,1), xlim=c(0,50))
  ...
  write.table(cbind(hz$mids, hz$counts, hz_$counts), sprintf("/data/Jakku/germany-height/histograms/cv/%s", gsub(".til", ".txt", basename(ftiles[site]))), row.names=FALSE, col.names=FALSE, sep=" ")
